...
class Name(Expr):
    ...
    def eval(self, env):
        ...
        if self.string in env:
            ...
            raise NameError('Variable not defined in environment.')
...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        eval_operands = [operands.eval(env) for operands in self.operands]
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        env_copy.update({parameter:arguments.pop(0) for parameter in self.parameters})

...
# END Problem 13
    ...
    def action(self, colony):
        ...
        while position:
            if position.ant:
                if position.ant not in self.reinforced_ants: 
                    position.ant.damage = position.ant.damage*2
                    self.reinforced_ants += [position.ant]
                if position.ant.container:
                    if position.ant.ant and (position.ant.ant not in self.reinforced_ants):
                        position.ant.ant.damage = position.ant.ant.damage*2
                        self.reinforced_ants += [position.ant.ant]
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0 and not self.can_remove:
            bees_win()

...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        assignment = bindings.first
        check_form(assignment, 2, 2)
        symbols = Pair(assignment.first, symbols)
        values = Pair(scheme_eval(assignment.second.first,env), values)
...
def do_define_macro(expressions, env):
    ...
    # BEGIN Problem 21
    check_form(expressions, 2)
    if scheme_listp(expressions.first) and scheme_symbolp(expressions.first.first):
